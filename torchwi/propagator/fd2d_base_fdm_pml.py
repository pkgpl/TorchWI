...
def impedance_matrix_vpad(omega, vel, h, npml, mat='csr', dtype=np.complex64):
    return impedance_matrix(omega, pml_pad(vel, npml), h, npml, mat, dtype)


def impedance_matrix(omega, vel, h, npml, mat='csr', dtype=np.complex64):
    nx, ny = vel.shape
    nxy = nx * ny
...
@jit
def assemble_coo(omega, v, h, dampx, dampy, dtype=np.complex64):
    nx, ny = v.shape
    xix = 1. - 1.0j * dampx / omega.real #np.abs(omega)  # omega.real
    xiy = 1. - 1.0j * dampy / omega.real #np.abs(omega)  # omega.real
    oh2 = 1. / h ** 2
    omega2 = omega ** 2
